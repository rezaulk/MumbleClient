...
namespace MurmurCommon
{
    public interface IVirtualServer : IDisposable
    {
        ...
        void StartListening(int sessionID, int channelID);
        void StopListening(int sessionID, int channelID);
        bool IsListening(int sessionID, int channelID);
        int[] GetListeningChannels(int userID);
        int[] GetListeningUsers(int channelID);
    }
}