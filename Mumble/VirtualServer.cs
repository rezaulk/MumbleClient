...
namespace Mumble
{
    public class VirtualServer : IVirtualServer, IDisposable
    {
        ...
        public int RegisterUser(VirtualServerEntity.User user)
        {
            ...
            // set user texture
            if (user.Texture != null  && user.Texture.Length > 0)
        ...
        public void UpdateUserInfo(VirtualServerEntity.User user)
        {
            ...
            {
                newInfo.Add((UserInfo)it.Key, it.Value);
            }
        ...
        public void UpdateUserState(VirtualServerEntity.OnlineUser user)
        {
            ...
            var state = new User(user.Session, user.Id, user.Mute, user.Deaf, user.Supress,
                ...
                user.Recording, 
                user.ChannelId, user.Name, user.OnlineSecs, user.BytesPerSec, user.Version, user.Release,
                user.Os, user.OsVersion, user.Identity, user.Context, user.Comment, user.Address, user.TcpOnly, user.Idlesecs , user.UdpPing,   user.TcpPing );
...
#region BANS
        ...
        public void StartListening(int sessionID, int channelID)
        {
            _server.startListening(sessionID, channelID);
        }

        public void StopListening(int sessionID, int channelID)
        {
            _server.stopListening(sessionID, channelID);
        }

        public bool IsListening(int sessionID, int channelID)
        {
           return  _server.isListening(sessionID, channelID);
        }
        public int[] GetListeningChannels(int userID)
        {
            return _server.getListeningChannels(userID);
        }

        public int[] GetListeningUsers( int channelID)
        {
            return _server.getListeningUsers(channelID);
        }
        #endregion
    }
