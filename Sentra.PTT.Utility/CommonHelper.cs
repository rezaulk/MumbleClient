...
namespace Sentra.PTT.Utility
{
    ...
    public partial class CommonHelper
    {
        ...
        /// <returns></returns>
        public static Tuple<HttpStatusCode, string> ValidateHttpReqMethod(params Tuple<object, string, int, object>[] tuples)
        {
            foreach (var item in tuples)
            {
                ...
                {
                    case 0:
                        return new Tuple<HttpStatusCode, string>(HttpStatusCode.BadRequest, item.Item2);
                    case (int)PTTEnums.ValidationType.IsNullOrEmpty:
                        if (item.Item1 is string var1 && string.IsNullOrEmpty(var1))
                            goto case 0;
                        break;
                    case (int)PTTEnums.ValidationType.IsPositiveIntegreOrNull:
                        if (!IsPositiveIntegreOrNull(item.Item1))
                            goto case 0;
                        break;
                    case (int)PTTEnums.ValidationType.IsLessThanOrEqual:
                        if (item.Item1 is int var2 && var2 <= 0)
                            goto case 0;
                        break;
                    case (int)PTTEnums.ValidationType.IsLessThanThree:
                        if (IsLengthLessThan((string)item.Item1))
                            goto case 0;
                        break;
                    case (int)PTTEnums.ValidationType.IsGreaterThanFifty:
                        if (IsLengthGreaterThanFifty((string)item.Item1))
                            goto case 0;
                        break;
                    case (int)PTTEnums.ValidationType.IsGreaterThanOrEqual:
                        if (!IsGreaterThanOrEqual(item.Item1))
                            goto case 0;
                        break;
                    case (int)PTTEnums.ValidationType.IsGreaterThan:
                        if (!IsGreaterThan(item.Item1))
                            goto case 0;
                        break;
                    case (int)PTTEnums.ValidationType.IsValidDate:
                        if (!IsValidDate(item.Item1))
                            goto case 0;
                        break;
                    default:
                        break;
        ...
        /// </param>
        /// <returns></returns>
        public static List<Tuple<HttpStatusCode, string>> ValidateHttpReqMethodList(params Tuple<object, string, int, object>[] tuples)
        {
            var returnTuple = new List<Tuple<HttpStatusCode, string>>();
            foreach (var item in tuples)
            {
                switch (item.Item3)
                {
                    case 0:
                        returnTuple.Add(new Tuple<HttpStatusCode, string>(HttpStatusCode.BadRequest, item.Item2));
                        break;
                    //return new Tuple<HttpStatusCode, string>(HttpStatusCode.BadRequest, item.Item2);
                    case (int)PTTEnums.ValidationType.IsNullOrEmpty:
                        if (item.Item1 is string var1 && string.IsNullOrEmpty(var1))
                    ...
                    case (int)PTTEnums.ValidationType.IsLessThanOrEqual:
                        if (item.Item1 is int var2 && var2 <= 0)
                            goto case 0;
                        break;
                    case (int)PTTEnums.ValidationType.IsLengthLessThan:
                        if (item.Item1 is string var3 && var3.Length <= 2)
                            goto case 0;
                        break;
                    default:
                        break;
        ...
        public static string GetNistTime()
        {
            ...
            {
                Console.WriteLine("Common Helper :------------ +++++++++++++++++"+ex.Message.ToString());
                throw ex;
            }
        ...
        #region
        public static bool IsLengthLessThan(string text)
        {
        ...
        public static bool IsLengthGreaterThanFifty(string text)
        {
            if (string.IsNullOrEmpty(text))
                return false;
        ...
        public static bool IsPositiveIntegreOrNull(object obj)
        {
            return ((obj is int && (int)obj >= 0) || obj is null) ? true : false;
        }
        public static bool IsGreaterThanOrEqual(object obj)
        {
            return ((obj is int) && (int)obj >= 0) ? true : false;
        }
        public static bool IsGreaterThan(object obj)
        {
            return ((obj is int) && (int)obj > 0) ? true : false;
        }
        public static bool IsValidDate(object obj)
        {
            return ((obj is DateTime) ? true : false);
        }
